...
PDFSIGN = (function () {
	...
	function findSuccessorEntry(xrefEntries, current) {
		...
		for (var i in xrefEntries) {
			if (xrefEntries[i].offset > currentOffset
				&& xrefEntries[i].offset < currentMin) {
	...
	/**
	 *
	 * @param xrefEntries {array} - 所有对象数组，下标对应对象编号
	 * @param used {array} - 使用的对象编号
	 * */
	function findFreeXrefNr(xrefEntries, used) {
		used = typeof used !== 'undefined' ? used : [];
		var inc = used.length;
		// console.log("---in findFreeXrefNr-xrefEntries--", xrefEntries)
		// console.log("---in findFreeXrefNr-used--", used)
		for (var i = 1; i < xrefEntries.length; i++) {
			var index = used.indexOf(i);
			var entry = xrefEntries["" + i];
			// 查找现有对象中是否有used中的对象编号
			// 如果没有，判断现有对象中是否有空对象，如果有，或者free属性为true，则返回i
			// entry.free代表？
			if (index === -1 && (typeof entry === 'undefined' || entry.free)) {
				return i;
			}
			// used中的编号存在于现有对象编号中，inc减1
			if (index !== -1) {
				inc--;
			}
		}
	...
	function updateXrefOffset(xref, offset, offsetDelta) {
		...
		for (var i in xref.xrefBlocks) {
			if (xref.xrefBlocks[i] >= offset) {
				// console.log("----xref.xrefBlocks[i] >= offset----", offsetDelta)
	...
	async function newSigStamper(pdf, root, rootSuccessor, date, cert, password, stampInfo, imageTools) {
		var array = pdf.stream.bytes; // 原始PDF数据
		// console.log("_---->>>>>pdf.stream===", pdf.stream)
		var newUsedEntries = []

		// 获取annot入口偏移量 - 作为签名证书的编号
		var annotEntry = findFreeXrefNr(pdf.xref.entries, newUsedEntries);
		/** 查找放置AcroForm的偏移量  解析由jspdf生成出来的PDF文件，find第三个参数不传  **/
			// var offsetForm = find(pdf.stream.bytes, '<<', root.offset) + 2;
			// var rectEntry = annotEntry + 1; // rect对象编号
		var rectEntry = annotEntry + 6; // rect对象编号
		var signImg = stampInfo.data;
		if(signImg) {
			rectEntry++;
		}

		var offsetForm = find(array, '<<', root.offset) + 2;
		// 获取页面信息
		var pages = pdf.catalog.catDict.get('Pages');

		// 获取第一页信息
		var pagesInfo = pages.get('Kids');
		var paginationInfo = pagesInfo[stampInfo.pageNo];
		var pageObjId = paginationInfo.num;
		var ref = pagesInfo[0];
		var xref = pdf.xref.fetch(ref);
		// find the stamper location pagination.
		// find pagination start offset
		var paginationOffset = find(array, `\n${pageObjId} 0 obj`);
		var offsetContent = find(array, '/Contents', paginationOffset);

		// var offsetContent = findBackwards(pdf.stream.bytes, '/Contents', offsetContentEnd);

		// 首先添加AcroForm对象 Fields 后面跟的是rect对象的编号
		var appendAcroForm = `/AcroForm<</Fields[${rectEntry} 0 R] `;

		appendAcroForm += '/SigFlags 3>>'

		var appendAnnots = '/Annots[' + rectEntry + ' 0 R]\n ';

		// 将acromForm插入到pdf 对象中 文件中
		...
		var offsetSig = offsetAnnot;
		var sigEntry = findFreeXrefNr(pdf.xref.entries, newUsedEntries);
		...
		var middle = '>/Filter/Adobe.PPKLite/Type/Sig/SubFilter/adbe.pkcs7.detached' +
			'/Location()/M(D:' + now(date) + '\')\n/ByteRange ';
		var byteRange = '[0000000000 0000000000 0000000000 0000000000]';
		var end = '/Reason()/ContactInfo()>>\nendobj\n\n';

		var append2 = start + crypto + middle + byteRange + end;
		var offsetByteRange = start.length + crypto.length + middle.length;

		newUsedEntries.push(offsetSig)
		var XObject1Entry = sigEntry + 1;
		var XObject2Entry = sigEntry + 2;
		var XObject3Entry = sigEntry + 3;
		var XObject4Entry = sigEntry + 4;
		var XObject5Entry = sigEntry + 5;
		var XObject6Entry = sigEntry + 6;
		var XObject7Entry = sigEntry + 8;
		var XObject8Entry = sigEntry + 9;
		var filters = ['FlateDecode']
		// 图片签章总共用到10个对象
		let SMaskCompress = signImg.smaskData;
		//1、 先添加第4个对象 - 图片 SMask
		var compression = 'MEDIUM'
		/*********Length 为FlateEncode 之后的长度********/
		var XObject3 = `${XObject3Entry} 0 obj\n` +
			`<</ColorSpace/DeviceGray/Subtype/Image/Height ${stampInfo.originH}/Filter/FlateDecode` +
			`/Type/XObject/Width ${stampInfo.originW}/Length ${SMaskCompress.length}/BitsPerComponent 8>>`;
		XObject3 += `stream\n`
		XObject3 += SMaskCompress.data;
		XObject3 += `\nendstream`;
		XObject3 += `\nendobj\n\n`;
		var offsetXObject3 = offsetAnnot;
		array = insertIntoArray(array, offsetXObject3, XObject3);
		// console.log("===do compressImg ", SMaskCompress)
		// 2、 添加第5个对象 - 图片流 - add Image
		var imgCompress = signImg.imgData;
		var XObject4 = `${XObject4Entry} 0 obj\n` +
			`<</ColorSpace/DeviceRGB/Intent/Perceptual/Subtype/Image/Height ${stampInfo.originH}` +
			`/Filter/FlateDecode/Type/XObject/Width ${stampInfo.originW}/Length ${imgCompress.length}` +
			`/SMask ${XObject3Entry} 0 R/BitsPerComponent 8>>` +
			`stream\n${imgCompress.data}` +
			`\nendstream` +
			`\nendobj\n`;
		var offsetXObject4 = offsetXObject3 + XObject3.length;
		array = insertIntoArray(array, offsetXObject4, XObject4);

		// 3、 添加第8个对象 Rect对象
		var append = `${rectEntry} 0 obj\n`+
			`<</P ${pageObjId} 0 R/Subtype/Widget/T(${rectEntry})/V ${sigEntry} 0 R/F 132` +
			`/Type/Annot/FT/Sig/DR<</XObject<</FRM ${XObject5Entry} 0 R>>>>` +
			`/Rect[${stampInfo.x} ${stampInfo.y} ${stampInfo.x + stampInfo.width} ${stampInfo.y + stampInfo.height}]` +
			`/AP<</N ${XObject6Entry} 0 R>>` +
			'>>\nendobj\n';``
		var offsetRect = offsetXObject4 + XObject4.length;
		// 先插入Rect对象 - Rect对象一定要放在签名对象之前 - this object must before sig object - XObject7Entry
		array = insertIntoArray(array, offsetRect, append)

		// 4、添加第1个对象 签名对象
		offsetSig = offsetRect + append.length
		array = insertIntoArray(array, offsetSig, append2);

		// 5、添加第9个对象
		var XObject7 = `${XObject7Entry} 0 obj\n` +
			`<</Name/Helv/Subtype/Type1/Type/Font/BaseFont/Helvetica/Encoding/WinAnsiEncoding>>` +
			`\nendobj\n`;
		var offsetXObject7 = offsetSig + append2.length;
		array = insertIntoArray(array, offsetXObject7, XObject7);

		// 6、添加第10个对象
		var XObject8 = `${XObject8Entry} 0 obj\n` +
			`<</Name/ZaDb/Subtype/Type1/Type/Font/BaseFont/ZapfDingbats>>` +
			`\nendobj\n`;
		var offsetXObject8 = offsetXObject7 + XObject7.length;
		array = insertIntoArray(array, offsetXObject8, XObject8);

		// 7、添加第6个对象
		// 插入XObject5
		var object5Stream = {
			data: `q 1 0 0 1 0 0 cm /n0 Do Q
q 1 0 0 1 0 0 cm /n2 Do Q\n`
		}
		for(let i = object5Stream.data.length; i < 512; i++) {
			object5Stream.data += '\u0000'
		}
		object5Stream = imageTools.processDataByFilters(object5Stream.data, filters)
		var XObject5 = `${XObject5Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Resources<</XObject<<` +
			`/n0 ${XObject1Entry} 0 R/n2 ${XObject2Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/FormType 1/Matrix [1 0 0 1 0 0]/Length ${object5Stream.data.length}>>`

		XObject5 += `stream\n`
		XObject5 += object5Stream.data
		XObject5 += `\nendstream`
		XObject5 += `\nendobj\n\n`;
		var offsetXObject5 = offsetXObject8 + XObject8.length;
		array = insertIntoArray(array, offsetXObject5, XObject5);

		// 8、添加第3个对象 - 设置图片可视区域
		var object2Stream = {
			data:  `q
${stampInfo.width} 0 0 ${stampInfo.height} 0 0 cm 
/img1 Do 
Q\n`
		}
		for(let i = object2Stream.data.length; i < 512; i++) {
			object2Stream.data += '\u0000'
		}
		object2Stream = imageTools.processDataByFilters(object2Stream.data, filters)
		var XObject2 = `${XObject2Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Matrix [1 0 0 1 0 0]/FormType 1/Resources<</XObject<<` +
			`/img0 ${XObject3Entry} 0 R/img1 ${XObject4Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/Length ${object2Stream.data.length}>>`

		XObject2 += `stream\n`;
		XObject2 += object2Stream.data;
		XObject2 += `\nendstream`;
		XObject2 += `\nendobj\n\n`;
		var offsetXObject2 = offsetXObject5 + XObject5.length;
		array = insertIntoArray(array, offsetXObject2, XObject2);

		// 9、添加第2个对象
		let bsBlankStr = {
			data: '% DSBlank\n'
		};
		for(let i = bsBlankStr.data.length; i < 512; i++) {
			bsBlankStr.data += '\u0000'
		}
		bsBlankStr = imageTools.processDataByFilters(bsBlankStr.data, filters)
		var XObject1 = `\n${XObject1Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Matrix [1 0 0 1 0 0]/FormType 1/Resources<<>>` +
			`/BBox[0 0 100 100]/Length ${bsBlankStr.data.length}>>` +
			`stream\n` + bsBlankStr.data +
			'\nendstream' +
			`\nendobj\n`
		var offsetXObject1 = offsetXObject2 + XObject2.length;
		array = insertIntoArray(array, offsetXObject1, XObject1)

		// 10、添加第7个对象
		var XObject6Stream = {
			data: 'q 1 0 0 1 0 0 cm /FRM Do Q\n'
		};
		for(let i = XObject6Stream.data.length; i < 512; i++) {
			XObject6Stream.data += '\u0000'
		}
		XObject6Stream = imageTools.processDataByFilters(XObject6Stream.data, filters)
		var XObject6 = `${XObject6Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Matrix [1 0 0 1 0 0]/FormType 1/Resources<<` +
			`/XObject<</FRM ${XObject5Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/Length ${XObject6Stream.data.length}>>` +
			`stream\n` + XObject6Stream.data +
			`\nendstream` +
			`\nendobj\n\n`;
		var offsetXObject6 = offsetXObject1 + XObject1.length;
		array = insertIntoArray(array, offsetXObject6, XObject6);

		let totalOffset = append2.length + append.length +
			XObject1.length + XObject2.length + XObject3.length +
			XObject4.length + XObject5.length + XObject6.length +
			XObject7.length + XObject8.length;
		updateXrefOffset(pdf.xref, offsetAnnot, totalOffset);

		var xrefBlocks = findXrefBlocks(pdf.xref.xrefBlocks);
		for (var i in xrefBlocks) {
			var oldSize = array.length;
			// remove old xref data
		...
		pdf.xref.entries[XObject1Entry] = {offset: offsetXObject1, gen: 0, free: false};
		pdf.xref.entries[XObject2Entry] = {offset: offsetXObject2, gen: 0, free: false};
		pdf.xref.entries[XObject3Entry] = {offset: offsetXObject3, gen: 0, free: false};
		pdf.xref.entries[XObject4Entry] = {offset: offsetXObject4, gen: 0, free: false};
		pdf.xref.entries[XObject5Entry] = {offset: offsetXObject5, gen: 0, free: false};
		pdf.xref.entries[XObject6Entry] = {offset: offsetXObject6, gen: 0, free: false};
		pdf.xref.entries[XObject7Entry] = {offset: offsetXObject7, gen: 0, free: false};
		pdf.xref.entries[XObject8Entry] = {offset: offsetXObject8, gen: 0, free: false};
		pdf.xref.entries[rectEntry] = {offset: offsetRect, gen: 0, free: false};
	...
	function appendSigStamper(pdf, root, rootSuccessor, date, cert, password, stampInfo, imageTools) {
		...
		// find AcroForm offset
		var offsetAcroForm = find(array, '/AcroForm<<', startRoot);
		var offsetFields = find(array, '/Fields', offsetAcroForm);
		// console.log("===>>>>offsetAcroForm>>", offsetFields)
		var endOffsetAcroForm = find(array, ']', offsetFields);

		var annotEntry = findFreeXrefNr(pdf.xref.entries);
		var sigEntry = findFreeXrefNr(pdf.xref.entries, [annotEntry]);

		var filters = ['FlateDecode']
		/**** defined stamper entry ****/
		var XObject1Entry = annotEntry;
		var XObject4Entry = annotEntry + 1;
		var XObject5Entry = annotEntry + 2;
		var XObject8Entry = annotEntry + 3;
		var XObject9Entry = annotEntry + 4;
		var XObject10Entry = annotEntry + 5;
		var XObject6Entry = annotEntry + 6;
		var XObject3Entry = annotEntry + 7;
		var XObject2Entry = annotEntry + 8;
		var XObject7Entry = annotEntry + 9;
		var appendAnnot = XObject8Entry + ' 0 R';
		// add annotEntry to Fields
		...
		//get first page, we have hidden sig, so don't bother
		var pagesInfo = pages.get('Kids');
		var sigPageNum = pagesInfo[stampInfo.pageNo];
		var pageObjId = sigPageNum.num;
		var contentRef = pagesInfo[0];
		var xref = pdf.xref.fetch(contentRef);

		// find the stamper location pagination.
		// find pagination start offset
		var paginationOffsetStart = find(array, `\n${pageObjId} 0 obj`);
		var paginationOffsetEnd = find(array, 'endobj', paginationOffsetStart) + 7;
		var startContent = array.length;
		array = copyToEnd(array, paginationOffsetStart, paginationOffsetEnd);
		// 查找是否有Annots
		var annotsOffset = find(array, '/Annots[', paginationOffsetStart, paginationOffsetEnd)
		var offsetAnnotRelative = 0
		if(annotsOffset == -1) {
			// if have no Annots , add annots before Contents
			var offsetContent = find(array, '/Contents', paginationOffsetStart);
			appendAnnot = '/Annots[' + appendAnnot + ']'
			offsetAnnotRelative = paginationOffsetEnd - offsetContent;
		} else {
			// already have Annots, fint end of ]
			var endOffsetAnnot = find(array, ']', annotsOffset, paginationOffsetEnd);
			offsetAnnotRelative = paginationOffsetEnd - endOffsetAnnot
			appendAnnot = ' ' + appendAnnot
		}
		...
		var signImg = stampInfo.data;
		let SMaskCompress = signImg.smaskData;
		//1、 先添加第4个对象 - 图片 SMask  - （add 4th object ）
		/*********Length 为FlateEncode 之后的长度********/
		var XObject4 = `${XObject4Entry} 0 obj\n` +
			`<</ColorSpace/DeviceGray/Subtype/Image/Height ${stampInfo.originH}/Filter/FlateDecode` +
			`/Type/XObject/Width ${stampInfo.originW}/Length ${SMaskCompress.length}/BitsPerComponent 8>>`;
		XObject4 += `stream\n`
		XObject4 += SMaskCompress.data;
		XObject4 += `\nendstream`;
		XObject4 += `\nendobj\n\n`;
		var offsetXObject4 = array.length;
		array = insertIntoArray(array, offsetXObject4, XObject4);

		// 2、 添加第5个对象 - 图片流 - add Image
		var imageData = signImg.imgData;
		var XObject5 = `${XObject5Entry} 0 obj\n` +
			`<</ColorSpace/DeviceRGB/Intent/Perceptual/Subtype/Image/Height ${stampInfo.originH}` +
			`/Filter/FlateDecode/Type/XObject/Width ${stampInfo.originW}/Length ${imageData.length}` +
			`/SMask ${XObject4Entry} 0 R/BitsPerComponent 8>>` +
			`stream\n${imageData.data}` +
			`\nendstream` +
			`\nendobj\n`;
		var offsetXObject5 = array.length;
		array = insertIntoArray(array, offsetXObject5, XObject5);

		// 添加第8个对象 Rect对象 - add 8th object
		var append = `${XObject8Entry} 0 obj\n` +
			`<</P ${pageObjId} 0 R/Subtype/Widget/T(${XObject8Entry})/V ${XObject1Entry} 0 R/F 132` +
			`/Type/Annot/FT/Sig/DR<</XObject<</FRM ${XObject6Entry} 0 R>>>>` +
			`/Rect[${stampInfo.x} ${stampInfo.y} ${stampInfo.x + stampInfo.width} ${stampInfo.y + stampInfo.height}]` +
			`/AP<</N ${XObject7Entry} 0 R>>` +
			`>>\nendobj\n\n`;
		// var offsetXObject8 = array.length;
		var offsetXObject8 = array.length;
		array = insertIntoArray(array, offsetXObject8, append);

		// add 1st object  signature obj
		// var startSig = array.length;
		var start = XObject1Entry + ' 0 obj\n<</Contents <';
		var dummy = sign(convertUint8ArrayToBinaryString(cert), 'A', password);
		//TODO: Adobe thinks its important to have the right size, no idea why this is the case
		var crypto = new Array(round256(dummy.length * 2)).join('0');
		var middle = '>\n/Type/Sig/SubFilter/adbe.pkcs7.detached/Location()/M(D:' + now(date) + '\')\n/ByteRange ';
		...
		//all together
		var append2 = start + crypto + middle + byteRange + end;
		var offsetXObject1 = array.length;
		array = insertIntoArray(array, offsetXObject1, append2);

		// 添加第9个对象 - add 9th obj
		var XObject9 = `${XObject9Entry} 0 obj\n` +
			`<</Name/Helv/Subtype/Type1/Type/Font/BaseFont/Helvetica/Encoding/WinAnsiEncoding>>` +
			`\nendobj\n`;
		var offsetXObject9 = array.length;
		array = insertIntoArray(array, offsetXObject9, XObject9);

		// 6、添加第10个对象
		var XObject10 = `${XObject10Entry} 0 obj\n` +
			`<</Name/ZaDb/Subtype/Type1/Type/Font/BaseFont/ZapfDingbats>>` +
			`\nendobj\n`;
		var offsetXObject10 = array.length;
		array = insertIntoArray(array, offsetXObject10, XObject10);
...
q 1 0 0 1 0 0 cm /n2 Do Q\n`
		...
		for(let i = object6Stream.data.length; i < 512; i++) {
			object6Stream.data += '\u0000'
		}
		object6Stream = imageTools.processDataByFilters(object6Stream.data, filters)
		var XObject6 = `${XObject6Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Resources<</XObject<<` +
			`/n0 ${XObject2Entry} 0 R/n2 ${XObject3Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/FormType 1/Matrix [1 0 0 1 0 0]/Length ${object6Stream.data.length}>>`

		XObject6 += `stream\n`
		XObject6 += object6Stream.data
		XObject6 += `\nendstream`
		XObject6 += `\nendobj\n\n`;
		var offsetXObject6 = array.length;
		array = insertIntoArray(array, offsetXObject6, XObject6);
...
Q\n`
		...
		for(let i = object3Stream.data.length; i < 512; i++) {
			object3Stream.data += '\u0000'
		}
		object3Stream = imageTools.processDataByFilters(object3Stream.data, filters)
		var XObject3 = `${XObject3Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Matrix [1 0 0 1 0 0]/FormType 1/Resources<</XObject<<` +
			`/img0 ${XObject4Entry} 0 R/img1 ${XObject5Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/Length ${object3Stream.data.length}>>`

		XObject3 += `stream\n`;
		XObject3 += object3Stream.data;
		XObject3 += `\nendstream`;
		XObject3 += `\nendobj\n\n`;
		var offsetXObject3 = array.length;
		array = insertIntoArray(array, offsetXObject3, XObject3);
		...
		var XObject2 = `\n${XObject2Entry} 0 obj\n` +
			...
			'\nendstream' +
			`\nendobj\n`
		var offsetXObject2 = array.length;
		array = insertIntoArray(array, offsetXObject2, XObject2)
		...
		for(let i = XObject7Stream.data.length; i < 512; i++) {
			XObject7Stream.data += '\u0000'
		}
		XObject7Stream = imageTools.processDataByFilters(XObject7Stream.data, filters)
		var XObject7 = `${XObject7Entry} 0 obj\n` +
			`<</Subtype/Form/Filter/FlateDecode/Type/XObject/Matrix [1 0 0 1 0 0]/FormType 1/Resources<<` +
			`/XObject<</FRM ${XObject6Entry} 0 R>>>>/BBox[0 0 ${stampInfo.width} ${stampInfo.height}]` +
			`/Length ${XObject7Stream.data.length}>>` +
			`stream\n` + XObject7Stream.data +
			`\nendstream` +
			`\nendobj\n\n`;
		var offsetXObject7 = array.length;
		array = insertIntoArray(array, offsetXObject7, XObject7);
		...
		xrefEntries[sigPageNum.num] = {offset: startContent, gen: 0, free: false};
		xrefEntries[XObject1Entry] = {offset: offsetXObject1, gen: 0, free: false}; // sig 对象
		xrefEntries[XObject8Entry] = {offset: offsetXObject8, gen: 0, free: false}; // rect 对象
		xrefEntries[XObject2Entry] = {offset: offsetXObject2, gen: 0, free: false};
		xrefEntries[XObject3Entry] = {offset: offsetXObject3, gen: 0, free: false};
		xrefEntries[XObject4Entry] = {offset: offsetXObject4, gen: 0, free: false};
		xrefEntries[XObject5Entry] = {offset: offsetXObject5, gen: 0, free: false};
		xrefEntries[XObject6Entry] = {offset: offsetXObject6, gen: 0, free: false};
		xrefEntries[XObject7Entry] = {offset: offsetXObject7, gen: 0, free: false};

		xrefEntries[XObject9Entry] = {offset: offsetXObject9, gen: 0, free: false};
		xrefEntries[XObject10Entry] = {offset: offsetXObject10, gen: 0, free: false};
		...
		var from1 = 0;
		var to1 = offsetXObject1 + start.length;
		var from2 = to1 + crypto.length;
		var to2 = (array.length - from2) - 1;
		var byteRange = '[' + pad10(from1) + ' ' + pad10(to1 - 1) + ' ' + pad10(from2 + 1) + ' ' + pad10(to2) + ']';
	...
	var api = {
		signpdf: function (pdfRaw, cert, password, signImg, imageTools, date) {
			date = typeof date !== 'undefined' ? date : new Date();
			...
			// console.log("---->>>signImg in ===", signImg)
			// if (signImg instanceof ArrayBuffer) {
			//   signImg = new Uint8Array(signImg);
			// }
			// console.log("---->>>signImg after ===", signImg)
			let pdf = loadPdf(pdfRaw);
			// console.log("---->>> after loadPdf ===", pdf)
			var root = findRootEntry(pdf.xref);
			var rootSuccessor = findSuccessorEntry(pdf.xref.entries, root);
			if (!isSigInRoot(pdf)) {
				if(signImg) {
					return  newSigStamper(pdf, root, rootSuccessor, date, cert, password, signImg, imageTools);
				} else {
				...
				if(signImg) {
					return  appendSigStamper(pdf, root, rootSuccessor, date, cert, password, signImg, imageTools);
				} else {
